...
def _draw_hole(arr, rng, x, y, state, visibility):
    rim = [93, 108, 112]
    _disk(arr, x, y, 12, rim, 0.95)
    base = {
        "thin": [52, 82, 92],
        "medium": [78, 103, 111],
        "thick": [136, 150, 151],
        "broken": [42, 51, 55],
        "contaminated": [86, 78, 72],
    }[state]
    _disk(arr, x, y, 9, base, 0.98)
    if state == "thick":
        for _ in range(5):
            _disk(arr, x + int(rng.normal(0, 5)), y + int(rng.normal(0, 5)), int(rng.integers(1, 3)), [185, 195, 190], 0.6)
    elif state == "broken":
        _line(arr, (x - 7, y - 4), (x + 7, y + 5), 2, [18, 23, 25], 0.9)
        _line(arr, (x - 3, y + 7), (x + 5, y - 7), 1, [22, 27, 29], 0.9)
    elif state == "contaminated":
        for _ in range(7):
            col = [36, 32, 28] if rng.random() < 0.55 else [210, 196, 170]
            _disk(arr, x + int(rng.normal(0, 5)), y + int(rng.normal(0, 5)), int(rng.integers(1, 3)), col, 0.8)
    if visibility == "low_dose":
        _disk(arr, x, y, 10, [92, 102, 105], 0.25)


def _add_global_artifacts(arr, rng, visibility, ood_axis):
    if visibility == "frosty":
        for _ in range(45):
            _disk(arr, int(rng.integers(0, 160)), int(rng.integers(0, 160)), int(rng.choice([1, 1, 2])), [220, 228, 220], 0.45)
    elif visibility == "low_dose":
        arr[:] = 128 + (arr - 128) * 0.62
    elif visibility == "crowded":
        for _ in range(18):
            _disk(arr, int(rng.integers(0, 160)), int(rng.integers(0, 160)), int(rng.integers(2, 5)), [70, 75, 72], 0.5)

    if ood_axis == "carbon_shadow":
        _rect(arr, 0, int(rng.integers(0, 55)), 160, int(rng.integers(35, 90)), [22, 28, 30], 0.28)
    elif ood_axis == "beam_gradient":
        gradient = np.linspace(0.72, 1.16, 160)[None, :, None]
        arr[:] = arr * gradient
    elif ood_axis == "ice_rings":
        for r in [28, 47, 66]:
            _line(arr, (80 - r, 80), (80 + r, 80), 1, [172, 190, 194], 0.15)
...
def _draw_scene(rng, trace):
    ...
    arr[:] = np.array([35, 43, 45]) + rng.normal(0, 2.0, arr.shape)
    ...
    if trace["crack_orientation"] == "horizontal":
        _line(arr, (8, int(rng.integers(72, 89))), (152, int(rng.integers(72, 89))), 3, [18, 21, 22], 0.85)
    elif trace["crack_orientation"] == "vertical":
        _line(arr, (int(rng.integers(72, 89)), 8), (int(rng.integers(72, 89)), 152), 3, [18, 21, 22], 0.85)
    elif trace["crack_orientation"] == "diagonal":
        _line(arr, (10, 16), (150, 144), 3, [18, 21, 22], 0.85)

    for _ in range(trace["fiducial_count"]):
        _disk(arr, int(rng.integers(20, 142)), int(rng.integers(20, 142)), int(rng.integers(3, 5)), [235, 195, 79], 0.92)
...
def _question(qtype):
    return {
        ...
        "fiducial_count": "How many bright gold fiducial beads are visible?",
    }[qtype]
